(function(window){
    ...
    // 🎬 Album ảnh xuất hiện sau khi đoạn văn kết thúc
    function startAlbum() {
        // ⚠️ LƯU Ý: Đảm bảo các đường dẫn ảnh này là chính xác
        const album = [
            "image/anh1.jpg",
            "image/anh2.jpg",
            "image/anh3.jpg",
        ...
        let i = 0;
        // Lấy phần tử album đã có trong HTML
        const $albumBox = $('#albumBox');
        const $img = $('#albumImg');

        // Hiển thị album Box (dùng hiệu ứng fadeIn đã có)
        $albumBox.fadeIn(2000);

        // Bắt đầu vòng lặp ảnh
        setInterval(() => {
            i = (i + 1) % album.length;
            // Chuyển ảnh với hiệu ứng mờ dần
            $img.fadeOut(1000, function () {
                $img.attr('src', album[i]).fadeIn(1000);
            });
        }, 5000); // Thay ảnh mỗi 5 giây
    }
